...
namespace ArrAlgs
{
    ...
    */
    ...
    // All functions in this class may change the array.
    ...
    // All functions in this class keep the array unchanged.
    // Searching functions.
    public static class Srch    // 'Srch' stands for search.
    {
        // Returns the index of 'num' in 'arr'.
        // If 'num' is not in 'arr', returns -1.
        // 'arr' must be sorted in ascending order.
        public static int BinarySrchIter(int[] arr, int key)
        {
        ...
        public static int BinarySrchRec(int[] arr, int key)
        {
            return BinSrchRec(arr, key, 0, arr.Length);
        }
        // 'low' and 'high' must be indexes in 'arr'.
        private static int BinSrchRec(int[] arr, int key, int low, int high)
        {
            if (low > high)
                return -1;
            ...
            int mid = (low + high) / 2;
            if (key < arr[mid])
                return BinSrchRec(arr, key, low, mid - 1);
            else if (key > arr[mid])
                return BinSrchRec(arr, key, mid + 1, high);
            else
                return mid;
        }
    }
